...
export function useGameEngine() {
  ...
  const spawnEnemy = (canvasWidth: number) => {
    ...
    if (level >= 3 && rand > 0.8) type = 'HEAVY';
    else if (level >= 2 && rand > 0.6) type = 'FAST';

    enemiesRef.current.push(new Enemy(canvasWidth, ENEMY_CONFIGS[type]));
  };

  const spawnPowerup = (canvasWidth: number) => {
    const types: (keyof typeof POWERUP_CONFIGS)[] = ['TRIPLE_SHOT', 'SHIELD', 'REPAIR'];
    const type = types[Math.floor(Math.random() * types.length)];
    powerupsRef.current.push(new Powerup(canvasWidth, type));
  };
  ...
  const update = (time: number) => {
    ...
    if (player) {
      ...
      // Shooting
      if (keysRef.current.has(' ') && time % 150 < 20) {
    ...
    const spawnRate = Math.max(500, 2000 - (stats.level * 100));
